...
def upload():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            pid = cur.execute("SELECT id FROM products WHERE name = ?", (product_name,)).fetchone()["id"]
            cur.execute("INSERT INTO sales (product_id, date, quantity) VALUES (?, ?, ?)", (pid, date_val, quantity))
            inserted += 1
        conn.commit()
        conn.close()
        unique_products = df[col_vara].dropna().astype(str).unique().tolist()
        return jsonify({"success": True, "message": f"Laddade upp {inserted} poster", "products": unique_products})
...
def get_sales(product_name):
    conn = get_connection()
    rows = conn.execute(
        "SELECT s.date, SUM(s.quantity) AS quantity FROM sales s JOIN products p ON s.product_id = p.id WHERE p.name = ? GROUP BY s.date ORDER BY s.date LIMIT 60",
        (product_name,),
    ).fetchall()
    conn.close()
...
def update_stock(product_name):
    ...
    conn = get_connection()
    conn.execute("UPDATE products SET current_stock = ? WHERE name = ?", (stock, product_name))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
...
def demo():
    try:
        generate_demo_data()
        return jsonify({"success": True, "message": "Demo-data genererad"})
    except Exception as exc:
